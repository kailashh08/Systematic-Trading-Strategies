...
closePositions=function(day,equity,position){
  ...
  if (!is.null(position)) {
    ...
    closeshort1<-merge(shortposition,candidates,by="symbol")  # Close only if we have a position
    tempcloseshort <- subset(shortposition,return < -0.02)
    closeshort2 <- merge(tempcloseshort,subset(signals,signals$date==day),by="symbol")[,c(1:6)]
    names(closeshort2)[5]<-"closedate"                        # keep track of the close date so we
    names(closeshort2)[6]<-"price"
    closeshort <- rbind(closeshort1,closeshort2)
    closeshort <- closeshort[!duplicated(subset(closeshort, select = symbol)), ]
    ...
    closelong1<-merge(longposition,candidates,by="symbol")  # Close only if we have a position
    tempcloselong <- subset(longposition,return < -0.02)
    closelong2 <- merge(tempcloselong,subset(signals,signals$date==day),by="symbol")[,c(1:6)]
    names(closelong2)[5]<-"closedate"                        # keep track of the close date so we
    names(closelong2)[6]<-"price"
    closelong <- rbind(closelong1,closelong2)
    closelong <- closelong[!duplicated(subset(closelong, select = symbol)), ]
...
applyRules=function(currdate,equity,position){
  ...
  if (!is.null(close.results)) {                    # Did we actually close out any positions
    temp<-close.results[,c(1,2)]                     # if we we need to remove them from our open positions
...
reviewTrades=function(trades,tradetype) {
  ...
  while (!done) {
    View(trades)
    cat("\014")
    print("The first two screens correspond to details about opening long and short positions while the second set of screens correspond to closing positions for the day")
...
if(!is.null(tws)) {
  ...
  portfolio <- portfolio[,c("local","position","return")]
   # portfolio$return[1] <- -0.1
